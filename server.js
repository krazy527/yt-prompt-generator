...
app.post("/api/cut-video", upload.fields([
  { name: "videoFile", maxCount: 1 },
  { name: "cookiesFile", maxCount: 1 },
]), async (req, res) => {
  try {
    const { videoUrl, startTime, endTime, cookieMode: requestCookieMode, browserName } = req.body || {};
    ...
    if (!req.files?.videoFile && !videoUrl) {
      return res.status(400).json({ error: "Please upload a video file or provide a YouTube URL." });
    }
    ...
    if (!sourcePath && videoUrl) {
      ...
      try {
        ...
        if (effectiveCookieMode === "browser") {
          ...
          if (!cookieSource) {
            return res.status(400).json({ error: "Cookie file is required when cookie mode is set to file." });
          }
    ...
    try {
      ...
      return res.status(400).json({ error: "Source video could not be found for clipping." });
    }
